...
public abstract class TransactionCommons {
    ...
    public void verifyNarayanaDeclarativeApproachTransaction() {
        ...
        Assertions.assertEquals(90, miguelAccount.getAmount(), "Unexpected amount on source account.");

        AccountEntity lopeAccount = getAccount(ACCOUNT_NUMBER_LOPE);
        Assertions.assertEquals(110, lopeAccount.getAmount(), "Unexpected amount on source account.");

        JournalEntity miguelJournal = getLatestJournalRecord(ACCOUNT_NUMBER_MIGUEL);
        Assertions.assertEquals(10, miguelJournal.getAmount(), "Unexpected journal amount.");
    }
    ...
    @Tag("QUARKUS-2492")
    @Test
    public void verifyRollbackForNarayanaDeclarativeApproach() {
        TransferDTO transferDTO = new TransferDTO();
    ...
    @Tag("QUARKUS-2492")
    @Test
    public void smokeTestNarayanaDeclarativeTransactionTrace() {
    ...
    @Tag("QUARKUS-2492")
    @Test
    public void smokeTestMetricsNarayanaDeclarativeTransaction() {
    ...
    private void verifyRestRequestTraces(String operationName) {
        ...
        await().atMost(1, TimeUnit.MINUTES).pollInterval(Duration.ofSeconds(5)).untilAsserted(() -> {
            ...
            String traceBody = jaegerResponse.then().extract().body().asString();
            Assertions.assertTrue(traceBody.contains(operationName),
                    "Trace " + operationName + " Not found");
        });
    }
